...
var submit_btn = document.getElementById("butAddVideo");

var chkbox = [];
var lang = document.getElementsByName('language');
var lang_err = document.getElementById('languageErr');
...
function test(checkbox) {
    ...
    if (tick == 0) {
        lang_err.style.display = "block";
        submit_btn.disabled = true;
    }
    else {
        lang_err.style.display = "none";
...
function getOMdbMovie() {
    ...
    fetch('https://www.omdbapi.com/?t=' + title + '&apikey=cb20eeed')
        .then((res) => {
            return res.json();
        }).then((data) => {
